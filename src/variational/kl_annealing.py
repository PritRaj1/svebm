...
class KLAnnealer:
    ...
    def _compute_schedule(self) -> np.ndarray:
        ...
        step_size = (self.max_kl_weight - self.min_kl_weight) / (
            period * self.ratio_increase
        )

        for cycle in range(self.n_cycle):
            weight, step = self.min_kl_weight, 0
            while weight <= self.max_kl_weight and (
                int(step + cycle * period) < self.total_steps
            ):
    ...
    def __repr__(self) -> str:
        return (
            f"KLAnnealer(total_steps={self.total_steps}, n_cycle={self.n_cycle}, "
