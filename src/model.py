...
class SVEBM(L.LightningModule):
    ...
    def forward(self, batch, mode="train", gen_type="greedy"):
        ...
        return {
            'logits': logits, 'z_mu': z_mu, 'z_logvar': z_logvar, 'z': z,
            'cd': cd, 'mi': mi, 'hidden': hidden
        }
    ...
    def training_step(self, batch, batch_idx):
        ...
        kl = self.loss_struct.kl_div(self.ebm, batch['tgt_probs'], 
                                     z_mu_in, z_logvar_in).mean()
        
        total_loss = nll + kl_weight * (kl - outputs['cd']) - self.loss_struct.mi_weight * outputs['mi']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        total_loss = nll + (kl - outputs['cd']) - self.loss_struct.mi_weight * outputs['mi']
    ...
    def test_step(self, batch, batch_idx):
        ...
        refs_text = [[t] for t in ids_to_text_list(batch['targets'], pad_id, bos_id, eos_id)]
    ...
    def configure_optimizers(self):
        non_ebm_params = [p for n, p in self.named_parameters() if not n.startswith('ebm.')]
